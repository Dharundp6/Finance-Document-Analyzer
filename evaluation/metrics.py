...
class EvaluationResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            if len(self.expected_answer) > 200
            else self.expected_answer,
            "generated_answer": self.generated_answer[:200] + "..."
            if len(self.generated_answer) > 200
            else self.generated_answer,
...
class RAGMetrics:
    ...
    def calculate_accuracy(
        self, retrieved: list[str], relevant: list[str], total_documents: int
    ) -> float:
        """
    ...
    def calculate_ndcg(
        ...
        # Calculate ideal DCG
        ideal_scores = sorted([relevance_scores.get(doc, 0.0) for doc in relevant], reverse=True)
    ...
    def calculate_hit_rate(self, retrieved: list[str], relevant: list[str], k: int = None) -> float:
        """
        Hit Rate @ K
    ...
    def calculate_retrieval_metrics(
        self, retrieved: list[str], relevant: list[str], total_documents: int = 100
    ) -> RetrievalMetrics:
    ...
    def calculate_rouge(self, generated: str, reference: str) -> tuple[float, float, float]:
        """
    ...
    def calculate_answer_completeness(self, generated: str, expected: str) -> float:
        """
    ...
    def calculate_generation_metrics(
        self, generated: str, reference: str, query: str, context: str
    ) -> GenerationMetrics:
    ...
    def calculate_answer_correctness(self, generated: str, expected: str) -> float:
        """
        ...
        dot_product = sum(a * b for a, b in zip(gen_vec, exp_vec, strict=False))
        norm_gen = math.sqrt(sum(a * a for a in gen_vec))
        norm_exp = math.sqrt(sum(b * b for b in exp_vec))

        cosine = 0.0 if norm_gen == 0 or norm_exp == 0 else dot_product / (norm_gen * norm_exp)
    ...
    def calculate_context_precision(
        self, retrieved_chunks: list[str], relevant_chunks: list[str], answer: str
    ) -> float:
        """
        Context Precision
    ...
    def calculate_context_recall(self, retrieved_chunks: list[str], expected_answer: str) -> float:
        """
        Context Recall
    ...
    def calculate_end_to_end_metrics(
        ...
        return EndToEndMetrics(
            answer_correctness=self.calculate_answer_correctness(generated_answer, expected_answer),
            context_precision=self.calculate_context_precision(
                retrieved_chunks, relevant_chunks, generated_answer
            ),
            context_recall=self.calculate_context_recall(retrieved_chunks, expected_answer),
