...
class EvaluationReport:
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"   • Passed: {self.passed_tests} ({self.passed_tests / max(self.total_tests, 1) * 100:.1f}%)"
        )
...
class RAGEvaluator:
    ...
    def __init__(self, query_function, retrieval_function=None):
        """
    ...
    def evaluate_single(self, test_case: TestCase, timeout: float = 120.0) -> EvaluationResult:
        """
        ...
        try:
            ...
            if not test_case.relevant_chunk_ids and test_case.relevant_keywords:
                retrieval_metrics = self._estimate_retrieval_from_keywords(
                    retrieved_chunks, test_case.relevant_keywords
                )
            ...
            return EvaluationResult(
                ...
                end_to_end_metrics=EndToEndMetrics(latency_ms=(time.time() - start_time) * 1000),
                metadata={
                    "query_type": test_case.query_type,
                    "difficulty": test_case.difficulty,
                    "test_id": test_case.id,
    ...
    def _estimate_retrieval_from_keywords(
        self, retrieved_chunks: list[str], keywords: list[str]
    ) -> RetrievalMetrics:
    ...
    def evaluate_dataset(
        self, dataset: EvaluationDataset, verbose: bool = True
    ) -> EvaluationReport:
        """
    ...
    def _generate_report(
        self, dataset_name: str, results: list[EvaluationResult], total_time: float
    ) -> EvaluationReport:
        ...
        generation_metrics = {
            ...
            "answer_relevance": statistics.mean(
                [r.generation_metrics.answer_relevance for r in results]
            ),
            "faithfulness": statistics.mean([r.generation_metrics.faithfulness for r in results]),
            "completeness": statistics.mean(
                [r.generation_metrics.answer_completeness for r in results]
            ),
        }
        ...
        e2e_metrics = {
            "answer_correctness": statistics.mean(
                [r.end_to_end_metrics.answer_correctness for r in results]
            ),
            "context_precision": statistics.mean(
                [r.end_to_end_metrics.context_precision for r in results]
            ),
            "context_recall": statistics.mean(
                [r.end_to_end_metrics.context_recall for r in results]
            ),
        ...
        metrics_by_type = {}
        query_types = {r.metadata.get("query_type", "unknown") for r in results}
            ...
            if type_results:
                metrics_by_type[qtype] = {
                    ...
                    "avg_correctness": statistics.mean(
                        [r.end_to_end_metrics.answer_correctness for r in type_results]
                    ),
                    "avg_relevance": statistics.mean(
                        [r.generation_metrics.answer_relevance for r in type_results]
                    ),
                }

        # Metrics by difficulty
        metrics_by_difficulty = {}
        difficulties = {r.metadata.get("difficulty", "unknown") for r in results}
        ...
        for diff in difficulties:
            ...
            if diff_results:
                metrics_by_difficulty[diff] = {
                    ...
                    "avg_correctness": statistics.mean(
                        [r.end_to_end_metrics.answer_correctness for r in diff_results]
                    ),
                    "pass_rate": sum(
                        1 for r in diff_results if r.end_to_end_metrics.answer_correctness > 0.5
                    )
                    / len(diff_results),
                }
