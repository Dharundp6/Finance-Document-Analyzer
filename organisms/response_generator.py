...
from core.types import GeneratedResponse, QueryContext, RetrievalResult
...
class ResponseGenerator:
    ...
    def generate(
        ...
        return self._create_response(
            response_id, query_id, answer, retrieval_results, confidence, processing_time, analysis
        )

    def _create_direct_response(
        self, response_id: str, query_id: str, analysis: QueryAnalysis, processing_time: float
    ) -> GeneratedResponse:
    ...
    def _generate_with_retry(self, prompt: str, max_retries: int) -> str:
        ...
        raise GenerationError(f"Failed to generate response: {last_error}") from last_error
...
• Use the `help` command for more guidance"""
    ...
    def _calculate_confidence(
        self, retrieval_results: list[RetrievalResult] | None, analysis: QueryAnalysis
    ) -> float:
    ...
    def _create_response(
        ...
        return GeneratedResponse(
            ...
            processing_time=processing_time,
            token_usage={"prompt_tokens": 0, "completion_tokens": len(answer) // 4},
            metadata={
    ...
    def generate_with_streaming(
        self, query: str, context: str, query_context: QueryContext | None = None
    ):
