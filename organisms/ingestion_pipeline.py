...
from core.types import DocumentMetadata, ProcessingResult
...
class IngestionPipeline:
    ...
    def ingest_file(
        self, file_path: str | Path, metadata: dict[str, Any] | None = None, move_file: bool = False
    ) -> ProcessingResult:
        """
        ...
        result = self.document_processor.process(
            file_path, extract_tables=True, extract_financial_entities=True
        )
            ...
            if file_path.is_file() and file_path.suffix.lower() in extensions:
                try:
                    ...
                    results[file_path.name] = ProcessingResult(
                        document_id="", success=False, errors=[str(e)]
                    )
    ...
    def ingest_bytes(
        self, file_bytes: bytes, filename: str, metadata: dict[str, Any] | None = None
    ) -> ProcessingResult:
        """
    ...
    def list_documents(self) -> list[dict[str, Any]]:
        ...
        return [self.get_document_info(doc_id) for doc_id in self.document_registry]

    def get_stats(self) -> dict[str, Any]:
        """Get ingestion statistics."""
        vector_stats = self.vector_store.get_stats()

        return {**vector_stats, "registered_documents": len(self.document_registry)}
