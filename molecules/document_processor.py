...
from atoms.chunk_creator import ChunkingConfig, FinancialAwareChunker
from atoms.file_reader import FileReaderFactory
from atoms.financial_extractor import FinancialExtractor
from atoms.text_cleaner import CleaningConfig, TextCleaner
from config.logging_config import logger
from config.settings import settings
from core.exceptions import DocumentProcessingError, FileSizeExceededError, UnsupportedFileTypeError
...
class DocumentProcessor:
    ...
    def __init__(self, chunk_size: int = None, chunk_overlap: int = None):
        ...
        self.chunker = FinancialAwareChunker(
            ChunkingConfig(chunk_size=self.chunk_size, chunk_overlap=self.chunk_overlap)
        )
    ...
    def process(
        ...
        # Create metadata
        self._create_metadata(file_path, file_type, document_id)
        ...
        try:
            ...
            if file_type in [DocumentType.CSV, DocumentType.EXCEL]:
                ...
                chunks, tables = self._process_tabular(raw_content, document_id, file_type)
            else:
                # Handle text-based documents
                chunks = self._process_text_document(raw_content, document_id, file_type)

                if extract_tables and "tables" in raw_content:
                    tables = self._process_tables(raw_content["tables"], document_id)
    ...
    def _create_metadata(
        self, file_path: Path, file_type: DocumentType, document_id: str
    ) -> DocumentMetadata:
    ...
    def _process_text_document(
        self, raw_content: dict, document_id: str, file_type: DocumentType
    ) -> list[TextChunk]:
        ...
        if file_type == DocumentType.PDF:
            ...
            if full_text.strip():
                ...
                chunks = self.chunker.chunk(
                    cleaned_text, document_id=document_id, metadata={"source_type": "word"}
                )
            ...
            if text.strip():
                ...
                chunks = self.chunker.chunk(
                    cleaned_text, document_id=document_id, metadata={"source_type": "text"}
                )
        ...
        if file_type == DocumentType.CSV:
            ...
            for sheet in raw_content.get("sheets", []):
                ...
                sheet_chunks = self._table_to_chunks(
                    table, document_id, metadata={"sheet_name": sheet.get("sheet_name")}
                )
                chunks.extend(sheet_chunks)

        return chunks, tables

    def _process_tables(self, tables_data: list[dict], document_id: str) -> list[TableData]:
    ...
    def _table_to_chunks(
        self, table: TableData, document_id: str, metadata: dict = None
    ) -> list[TextChunk]:
        ...
        schema_chunk = TextChunk(
            ...
            document_id=document_id,
            metadata={**(metadata or {}), "table_id": table.table_id, "type": "schema"},
        )
        ...
        if markdown_table:
            ...
            if len(markdown_table) > self.chunk_size:
                ...
                for i, row in enumerate(table.rows):
                    row_content = " | ".join(
                        f"{h}: {v}" for h, v in zip(table.headers, row, strict=False)
                    )
                    row_chunk = TextChunk(
                        chunk_id=str(uuid.uuid4()),
                        content=row_content,
                        chunk_type=ChunkType.TABLE,
                        document_id=document_id,
                        metadata={**(metadata or {}), "table_id": table.table_id, "row_index": i},
                    )
                ...
                table_chunk = TextChunk(
                    ...
                    chunk_type=ChunkType.TABLE,
                    document_id=document_id,
                    metadata={**(metadata or {}), "table_id": table.table_id, "type": "full_table"},
                )
