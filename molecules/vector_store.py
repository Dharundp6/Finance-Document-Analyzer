...
from core.types import ChunkType, TextChunk
...
class FAISSVectorStore:
    ...
    def add_chunks(
        self, chunks: list[TextChunk], batch_size: int = 32, show_progress: bool = True
    ) -> int:
    ...
    def search(
        self, query: str, top_k: int = 10, filter_dict: dict[str, Any] | None = None
    ) -> list[dict[str, Any]]:
    ...
    def get_chunk(self, chunk_id: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "document_id": chunk.document_id,
            "page_number": chunk.page_number,
            "metadata": {"chunk_type": chunk.chunk_type, **chunk.metadata},
        }

    def delete_document(self, document_id: str) -> int:
        """Delete all chunks for a document."""
        chunks_to_delete = [
            faiss_id for faiss_id, chunk in self.chunks.items() if chunk.document_id == document_id
        ]
    ...
    def get_all_document_ids(self) -> list[str]:
        ...
        return list({chunk.document_id for chunk in self.chunks.values()})
