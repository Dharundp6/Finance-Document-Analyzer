...
from core.types import TableData, TextChunk, ChunkType, FinancialEntity
...
class TableNormalizer:
    ...
    def normalize_table(
        ...
        if detect_header and self._headers_look_like_data(normalized_headers):
...
class TableAnalyzer:
    ...
    def analyze_table(
        self,
        table: TableData,
        config: TableProcessingConfig = None
    ) -> Dict[str, Any]:
        """
    ...
    def _identify_key_metrics(
        ...
        important_keywords = [
            'total', 'net', 'gross', 'revenue', 'income', 'profit',
            'loss', 'ebitda', 'ebit', 'margin', 'growth', 'ratio'
        ]
...
class TableProcessor:
    ...
    def _create_schema_chunk(
        ...
        content_parts.append(f"Dimensions: {dims.get('rows', 0)} rows × {dims.get('columns', 0)} columns")
