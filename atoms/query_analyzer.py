...
class QueryAnalysis:
    ...
    direct_response: str | None = None  # For greetings, chitchat, etc.
...
class QueryAnalyzer:
    ...
    CHITCHAT_PATTERNS = [
        ...
        r"^who\s+are\s+you",
        r"^are\s+you\s+(a\s+)?(robot|ai|bot)",
    ...
    FAREWELL_PATTERNS = [r"^bye", r"^goodbye", r"^see\s+you", r"^take\s+care", r"^later$", r"^cya$"]
...
Or type `help` for more information.""",
    ...
    def analyze(self, query: str) -> QueryAnalysis:
        ...
        comparison_items = (
            self._extract_comparison_items(query_lower)
    ...
    def _check_non_document_query(self, query: str) -> QueryAnalysis | None:
    ...
    def _is_unclear_query(self, query: str) -> bool:
        ...
        words = query.split()
        return len(words) < self.MIN_MEANINGFUL_WORDS and not any(
            metric in query for metric in self.FINANCIAL_METRICS
        )
    ...
    def _is_specific_query(self, query: str, metrics: list[str], entities: list[str]) -> bool:
        ...
        general_patterns = [
            ...
            r"\bgive\s+me\s+information\b",
            r"\boverall\b",
        ]

        return all(not re.search(pattern, query) for pattern in general_patterns)
    ...
    def _requires_calculation(self, query: str) -> bool:
        ...
        return any(re.search(pattern, query) for pattern in calc_patterns)
