    ...
    def chunk(
        ...
        for i, chunk_text in enumerate(chunks):
            chunk = TextChunk(
                ...
                document_id=document_id,
                page_number=page_number,
                metadata={**(metadata or {}), "chunk_index": i, "chunking_method": "sentence"},
            )
            result.append(chunk)
...
class FinancialAwareChunker(BaseChunker):
    ...
    def chunk(
        ...
        for section_idx, section in enumerate(sections):
            section_metadata = {**(metadata or {}), "section_index": section_idx}
    ...
    def _split_by_sections(self, text: str) -> list[str]:
        ...
        for split in splits:
            if split and split.strip():
                ...
                is_header = any(
                    re.match(pattern, split.strip()) for pattern in self.SECTION_PATTERNS
                )
...
def create_chunks(
    text: str, document_id: str, strategy: str = "financial", **kwargs
) -> list[TextChunk]:
    """
